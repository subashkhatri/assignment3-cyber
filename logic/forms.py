...
class CreateForm(FlaskForm):
    name = StringField("Name of Account: ", validators=[
                       InputRequired(), Length(max=50)])
    balance = IntegerField("Opening balance (optional)", default=10)
    password = PasswordField(
        "Account password",
        validators=[InputRequired(), EqualTo(
            "pwd_confirm", message="Passwords must match"), Length(max=50)]
    )
    pwd_confirm = PasswordField(
        "Confirm account password", validators=[Length(max=50)])
    submit = SubmitField("Create Account")


class LoginForm(FlaskForm):
    id = StringField("Username: ", validators=[
                     InputRequired(), Length(max=50)])
...
class MessageForm(FlaskForm):
    message_text = StringField("Your Message: ", validators=[
...
class TransferForm(FlaskForm):
    account_id = IntegerField(
        "Recipient's Account ID: ", validators=[InputRequired()])
